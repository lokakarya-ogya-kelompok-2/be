...
public class AssessmentSummarySpecification {
    ...
    public Specification<AssessmentSummary> userFullNameContains(String substr) {
        return (root, query, cb) -> {
            Join<AssessmentSummary, User> assessmentSummaryUserJoin = root.join("user", JoinType.LEFT);
    ...
    public Specification<AssessmentSummary> divisionIdIn(List<UUID> divisionIds) {
        return (root, query, cb) -> {
            Join<AssessmentSummary, User> assessmentSummaryUserJoin = root.join("user", JoinType.LEFT);
            Join<User, Division> userDivisionJoin = assessmentSummaryUserJoin.join("division", JoinType.LEFT);
    ...
    public Specification<AssessmentSummary> approvalStatusEquals(Integer approvalStatus) {
        return spec.fieldEquals("approvalStatus", approvalStatus);
    }
}
